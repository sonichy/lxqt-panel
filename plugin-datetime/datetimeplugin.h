...
class DatetimePlugin : public QObject, public ILXQtPanelPlugin
{
...
public:
    DatetimePlugin(const ILXQtPanelPluginStartupInfo &startupInfo);
    ~DatetimePlugin();
    ...
    virtual QString themeId() const override {
        return QStringLiteral("DatetimePlugin");
    }

    // enum Flag {
    //     NoFlags              = 0,   ///< It does not have any properties set.
    //     PreferRightAlignment = 1,   /**< The plugin prefers right alignment (for datetime the clock plugin);
...
class DatetimeLibrary: public QObject, public ILXQtPanelPluginLibrary
{
    Q_OBJECT
...
public:
    ...
    {
        return new DatetimePlugin(startupInfo);
    }
};
